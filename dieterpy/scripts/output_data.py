...
from dieterpy.scripts.gdx_handler import gdx_get_set_coords, gdx_get_symb_info, gdx_get_symb_list, \
                                gdx_get_symb_recordnr, gdx_get_symb_recordnr_from_list, gdx_get_summary
...
def read_iteration_symbols(csvpath=None):
    ...
    # print(rawcsvdata)
    d = dict(zip(rawcsvdata.columns.levels[1], [i if 'Unnamed' not in i else '.' for i in rawcsvdata.columns.levels[1]]))
    ...
    for df in datalist:
        df = df.join(pd.DataFrame([0]*len(df), index=df.index, columns=pd.MultiIndex.from_product([['.'], ['.']])))
...
def gams_csvtovaex_parallel(L, queue, queue_lock, list_lock, scen_name, block, gams_dir, csv_bool, pickle_bool, vaex_bool, main_gdx, diffgdxfile=None):
    '''
    ...
    while True:
        ...
        if name[-6:] == '_Fixed':
            print(f'Symbol {name} has been ommited. It is created to handel {name[:-6]} variable for GUSS tool dict')
            continue
        ...
        dims_in_csv = [dim for dim in csv_header_list if dim not in common]  # see the csv headers and select headers apart of Val, Marginal ...
        csv_header_set = set(dims_in_csv)

        _, maingdx_info = gdx_get_symb_info(gams_dir=gams_dir, filename=main_gdx, symbol=name)
        gdx_dims_set = set(maingdx_info['dims'])
        if csv_header_set.issubset(gdx_dims_set):
            common_header = csv_header_list[maingdx_info['nrdims']:] # Val marginal and so on
            headers = maingdx_info['dims'] + [hd for hd in common_header if hd in selected]
            ...
            if vaex_bool:
                try:
                    import vaex
                    df_pandas = df_pandas.reindex(columns = sets_list + selected + ['Symbol', 'Block', 'ID'])
            ...
            if diffgdxfile is None:
                raise Exception(f'Symbol {name} in csv has different headers than the file {main_gdx}. You must provide diffgdxfile')
            else:
                _, diffgdx_info = gdx_get_symb_info(gams_dir=gams_dir, filename=diffgdxfile, symbol=name)
            ...
            if maingdx_info['nrdims'] == diffgdx_info['nrdims']:
                ...
                common_header = csv_header_list[diffgdx_info['nrdims']:] # Val marginal and so on
                headers = csv_header_list[:diffgdx_info['nrdims']] + [hd for hd in common_header if hd in selected]
                new_headers = maingdx_info['dims'] + [hd for hd in common_header if hd in selected]
...
def GDXpostprocessing(method='direct', input=None, cores_data=0, sysdir=None, csv_bool=True, pickle_bool=True, vaex_bool=True, base=None):
    if not any([csv_bool, pickle_bool, vaex_bool]):
        ...
        if method == 'direct':
            if input is None:
                raise Exception(f'Wnen method is {method}, input must be a list of dictionaries. Dictionary is an output of solve functions')
            else:
                RUNS_list = input
        elif method == 'global':
            if input is None:
                raise Exception(f'Wnen method is {method}, input must be a string path that match glob format. e.g "/user/project1/output_data/*/*_config.yml"')
            else:
                RUNS_list = []
            ...
            if input is None:
                raise Exception(f'Wnen method is {method}, input must be a list of strings. Absolute path to the corresponding scenario file *_config.yml')
            else:
                RUNS_list = []
        ...
        for resultdc in RUNS_list:
            start = time.time()
            symbfile = os.path.join(settings.BASE_DIR_ABS, resultdc['reporting_symbols_file'].rpartition(resultdc['BASE_DIR_ABS'])[-1][1:])
            scen_name = resultdc['id']
            scen_dir_abspath = os.path.join(settings.BASE_DIR_ABS, resultdc['id_dir'].rpartition(resultdc['BASE_DIR_ABS'])[-1][1:])
            csv_dir_path = os.path.join(settings.BASE_DIR_ABS, resultdc['csv_dir'].rpartition(resultdc['BASE_DIR_ABS'])[-1][1:])
            os.makedirs(csv_dir_path, exist_ok=True)

            if resultdc['guss_tool']:
                gdxfilepath_tmp = os.path.join(settings.BASE_DIR_ABS, resultdc['tmp_gdx'].rpartition(resultdc['BASE_DIR_ABS'])[-1][1:])
            ...
            symbols_in_the_main_gdx = gdx_get_symb_list(gams_dir=sysdir, filename=main_gdx_file)  # gdxcc api
            features_in_the_main_gdx = gdx_get_set_coords(gams_dir=sysdir, filename=main_gdx_file, setname='feat_included')  # gdxcc api
            preferred_symbols = get_symb_from_features(feat_ingdx=features_in_the_main_gdx, symbfile=symbfile)  # those symbols indicated in the symbols.csv file
            print(os.path.basename(main_gdx_file), 'Columns from /settings/reporting_symbols.csv:\n', ['Basic'] + features_in_the_main_gdx)
            pre_selected_symbols = [symb for symb in symbols_in_the_main_gdx if symb in preferred_symbols]
            try:
                records = gdx_get_symb_recordnr_from_list(gams_dir=sysdir, filenamelist=[gdxfilepath, main_gdx_file], symbolnamelist=pre_selected_symbols)
                selected_symbols = [symb for symb, recNr in records.items() if recNr > 0]
            except:
                print('except')
        ...
        print(f'{all_symb_count} symbols through {ln} gdx files finished in {diffcsv} min')
        ...
        for resultdc in RUNS_list:
            ...
            with Manager() as manager:
                ...
                processes = {}
                for i in range(nr_workers):
                    processes[i] = Process(target=gams_csvtovaex_parallel, args=(L,
                                                                                queue,
                                                                                queue_lock,
                                                                                list_lock,
                                                                                scen_name,
            ...
            if pickle_bool:
                ...
                for symb_dict in listofsymbdicts:
                    symbols_dict[symb_dict['name']] = {'data':symb_dict['data'],
                                                        'dims': symb_dict['dims'],
                                                        'type': symb_dict['type'],
                ...
                print('  pickle done')
            print(' ',scen_name,'time:', str(round(time.time() - stime, 1)), 'seconds')
