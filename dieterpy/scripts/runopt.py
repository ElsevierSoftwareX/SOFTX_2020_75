...
from dieterpy.scripts.input_data import getConfigVariables, getGlobalFeatures, generateInputGDX, \
                                    prepareGAMSAPI, defineGAMSOptions, writeCountryOpt, \
                                    writeConstraintOpt, getIterableDataDict, genStringOptData, \
                                    genIterationDict, convert_par_var_dict, getGussVariables, \
                                    setCountryIteration, setDataIteration, createModelCheckpoint, \
                                    getConstraintsdata, getTopographydata, getGDXoutputOptions

from dieterpy.scripts.solve import scen_solve, guss_solve, guss_parallel
from dieterpy.scripts.output_data import GDXpostprocessing, solver_status_summary
...
def main():
    ...
    BASE['BASE_DIR_ABS']      = settings.BASE_DIR_ABS  # absolute path to child folder of project directory
    ...
    active_gf, gdx_abspaths_dc = generateInputGDX(BASE['INPUT_DIR_ABS'], BASE['BASE_DIR_ABS'], BASE['GDX_INPUT_ABS'], convar_dc)
    ...
    # Get iterable data
    data_it_dc, data_it_gdx = getIterableDataDict(BASE['ITERATION_DIR_ABS'], BASE['GDX_INPUT_ABS'], convar_dc)

    # Add data_it_gdx to gdx_abspaths_dc
    gdx_abspaths_dc['data_it_gdx'] = data_it_gdx

    ##### ITERATION CONFIG
    iteration_main_dict, list_constraints = genIterationDict(convar_dc, BASE['ITERATION_DIR_ABS'])

    # Get guss tool configuration
    guss, parallel, guss_threads = getGussVariables(convar_dc)
    # save into BASE
    BASE['guss_tool'], BASE['guss_parallel'], BASE['guss_threads'] = guss, parallel, guss_threads
    ...
    if activate_st:
        ...
        bar.progress(10/100)
    print('DIETER preparation finished. It took %s minutes' % (round((time.time() - start_time_global)/60  , 1)))
    ...
    for block, block_iter_dc in iteration_main_dict.items():
        ...
        BASE['tmp'][block] = {}
        BASE['tmp'][block]['str_block'] = '_b' + str(block).zfill(2)  # e.g. block=21 then after zfill(5) results 00021, five digits
        ...
        choosen_constraints = writeConstraintOpt(opt, block_iter_dc, list_constraints, itercon_dc)
        ...
        cp_file, main_gdx_file = createModelCheckpoint(ws, opt, cp, BASE['MODEL_DIR_ABS'],
                                                        BASE['RESULTS_DIR_ABS'], data_it_dc,
                                                        data_scen_key, BASE['tmp'][block]['str_block'])
        ...
        if guss:
            guss_symbol_block, symbs = convert_par_var_dict(symbols_dict=dict_parameters_block)
            if parallel:
                with Manager() as manager:
                    ...
                    if guss_threads == 0:
                        ...
                        print('GUSS_parallel_threads:', guss_threads, 'is greater than ', min(count,cores), '. Minimum value is selected')
                        nr_workers = min(count,cores)
                    ...
                    for i in range(nr_workers):
                        processes[i] = Process(target=guss_parallel, args=(block_results,
                                                                            queue,
                                                                            queue_lock,
                                                                            print_lock,
                                                                            symbs,
