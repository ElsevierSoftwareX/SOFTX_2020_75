...
def scen_solve(scen_run, base, run, block):
    ...
    tmp['summary'] = countries + '_' + constraints + '_' + data_scen + '_' + \
                        '_'.join([k + "_" + str(v) for k, v in symbs_dict.items()])
    tmp['summary_file'] = os.path.join(tmp['RESULTS_DIR_ABS'], tmp['id'] + '_' + str(tmp['modelstatus']) + '_' + \
                            str(round(tmp['system_costs']/1000000, 1)) + '_' + tmp['summary'] + '.txt')
    tmp['config'] = {k:v for k, v in base['tmp'][block]['runs'].items() if k not in ['par_var','run_nr']}
    ...
    tmp['reporting_symbols_file'] = os.path.join(base['SETTINGS_DIR_ABS'], 'reporting_symbols.csv')
    ...
    for symb, dcc in dc.items():
        ...
        for dom, v in dcc.items():
            if dom == ('.',):  # dimensionless symbol
                ...
                activation = []
                for i in range(info['nrdims']):  # info['dims'] -> info['nrdims'] e.g. ['n','sto'] ->  2
                    #print(dom, info['dims'][i])
                    if info['dims'][i] == dom[i]:  # dom looks like ('FR','sto')
                        activation.append(dom[i])
                    else:
                        activation.append('0')  # resulting activation list e.g. ['0','sto']
                matrix = []
                for j, setelem in enumerate(activation):
                    if setelem != '0':
                        matrix.append(gdx_get_set_coords(gams_dir=None, filename=maingdx, setname=setelem))  # e.g. ['sto1','sto2',...]
                    else:
                        matrix.append([dom[j]])  # e.g. ['FR']
                allcombination = [i for i in itertools.product(*matrix)]  # e.g. [('FR','sto1'), ('FR','sto2'),...]
                for coord in allcombination:
                    dcmodifier[symb][idx_modifier].add_record(coord).value = v  # v is always the same for all combinations. e.g. 'N_STO_P('FR','sto') = 100' then v = 100
    return dcmodifier
...
def guss_parallel(result, queue, queue_lock, print_lock, symbs, base, block):
    ...
    mi_opt = GamsModelInstanceOpt(solver="cplexd",opt_file=1, no_match_limit=999999999, debug=True)
    copyfile(os.path.join(tmp['RUN_DIR_ABS'],'cplexd.opt'), os.path.join(ws.working_directory, 'cplexd.opt'))

    dcmodifier = {}
    for sy in symbs:  # identify the symbols with different type and set modifier for each symbols
        if '.' in sy:
            raw_symb = sy.split('.')[0]
        ...
        if info['type'] == 'parameter':
            ...
            try:
                ...
                # print(f"{e}")
                symbol_instance = mi.sync_db.add_variable(raw_symb, dim, VarType.Positive)
            # add to mi.sync_db
            if update_action == 'lo':
                dcmodifier[sy] = [symbol_instance, UpdateAction.Lower, mi.sync_db.add_parameter(raw_symb+'_Lower', dim, txt+' MODIFIER Lower')]
            elif update_action == 'up':
                dcmodifier[sy] = [symbol_instance, UpdateAction.Upper, mi.sync_db.add_parameter(raw_symb+'_Upper', dim, txt+' MODIFIER Upper')]
            elif update_action == 'fx':
                dcmodifier[sy] = [symbol_instance, UpdateAction.Fixed, mi.sync_db.add_parameter(raw_symb+'_Fixed', dim, txt+' MODIFIER Fixed')]
            else:
                raise Exception(f'Update_action "{update_action}" for {sy} not valid')
        elif info['type'] == 'equation':
            raise Exception('To modify equations is not implemented yet')

    mi.instantiate("DIETER using lp min Z",
...
def guss_solve(queue, symbs, base, block):
    ...
    opt.solvelink = 0
    mi = cp.add_modelinstance()
    mi_opt = GamsModelInstanceOpt(solver="cplex",opt_file=1, no_match_limit=999999999, debug=True)
    copyfile(os.path.join(RUN_DIR_ABS,'cplex.opt'), os.path.join(ws.working_directory, 'cplex.opt'))
