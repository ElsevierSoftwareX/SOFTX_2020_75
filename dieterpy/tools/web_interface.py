...
try:
    ...
    st.beta_set_page_config(page_title='DIETERpy', page_icon=':power:', layout='wide', initial_sidebar_state='auto')
...
def page_settings(state):
    ...
    state.checkbox_proj_var = st.sidebar.checkbox("Project Variables", state.checkbox_proj_var, key='checkbox_proj_var')
...
def page_optimization(state):
    ...
    st.write("You can run DIETER by clicking the button 'Run optimization'. The optimization process may take a long time, depending on the size of the problem. If it occurs any issue, this will be displayed below for its debugging.")
...
def get_proj_var(state):
    ...
    state.scenarios_iteration = True if df[df['feature'] == 'scenarios_iteration']['value'].values[0].lower() == 'yes' else False
    state.skip_input = True if df[df['feature'] == 'skip_input']['value'].values[0].lower() == 'yes' else False
    state.skip_iteration_data_file = True if df[df['feature'] == 'skip_iteration_data_file']['value'].values[0].lower() == 'yes' else False
    state.base_year = int(df[df['feature'] == 'base_year']['value'].values[0])
    state.end_hour = df[df['feature'] == 'end_hour']['value'].values[0]
    state.dispatch_only = True if df[df['feature'] == 'dispatch_only']['value'].values[0].lower() == 'yes' else False
    state.network_transfer = True if df[df['feature'] == 'network_transfer']['value'].values[0].lower() == 'yes' else False
    state.no_crossover = True if df[df['feature'] == 'no_crossover']['value'].values[0].lower() == 'yes' else False
    state.GUSS = True if df[df['feature'] == 'GUSS']['value'].values[0].lower() == 'yes' else False
    state.GUSS_parallel = True if df[df['feature'] == 'GUSS_parallel']['value'].values[0].lower() == 'yes' else False
    state.GUSS_parallel_threads = int(df[df['feature'] == 'GUSS_parallel_threads']['value'].values[0])
    ...
    state.iteration_data_file = df[df['feature'] == 'iteration_data_file']['value'].values[0]
    state.gdx_convert_parallel_threads = int(df[df['feature'] == 'gdx_convert_parallel_threads']['value'].values[0])
    state.gdx_convert_to_csv = True if df[df['feature'] == 'gdx_convert_to_csv']['value'].values[0].lower() == 'yes' else False
    state.gdx_convert_to_pickle = True if df[df['feature'] == 'gdx_convert_to_pickle']['value'].values[0].lower() == 'yes' else False
    state.gdx_convert_to_vaex = True if df[df['feature'] == 'gdx_convert_to_vaex']['value'].values[0].lower() == 'yes' else False
    state.report_data = True if df[df['feature'] == 'report_data']['value'].values[0].lower() == 'yes' else False

def edit_proj_var(state):
    state.scenarios_iteration = st.checkbox("1. Run several scenarios", state.scenarios_iteration)
    state.skip_input = st.checkbox("2. Skip input excel to gdx conversion", state.skip_input)
    if not state.skip_input:
        state.data_input_file = st.text_input("2.a Data-input file name", state.data_input_file or "")
        state.time_series_file = st.text_input("2.b Time-series file name", state.time_series_file or "")
    state.skip_iteration_data_file = st.checkbox("3. Skip time-series scenario excel to gdx conversion", state.skip_iteration_data_file)
    if not state.skip_iteration_data_file:
        state.iteration_data_file = st.text_input("3.a Time-series scenario file name", state.iteration_data_file or "")
    option_year = [n for n in range(2010,2101,2)]
    state.base_year = st.selectbox("4. Select year of data (check time-series excel file)", option_year, option_year.index(state.base_year) if state.base_year else 0)
    state.end_hour = st.text_input("5. Select optimization end hour (check time-series excel file)", state.end_hour, 5)
    state.dispatch_only = st.checkbox("6. Model - dispatch only", state.dispatch_only)
    state.network_transfer = st.checkbox("7. Model - network transport", state.network_transfer)
    ...
    if state.GUSS:
        state.GUSS_parallel = st.checkbox("9.a Solve scenarios in parallel", state.GUSS_parallel)
        if state.GUSS_parallel:
            state.GUSS_parallel_threads = st.slider("Number of cores for parallel runs. To select the maximum number of cores choose zero.", 0, 10, state.GUSS_parallel_threads)

    state.gdx_convert_to_csv = st.checkbox("10. Convert symbols from GDX to CSV files", state.gdx_convert_to_csv)
    state.gdx_convert_to_vaex = st.checkbox("11. Convert symbols from GDX to VAEX files", state.gdx_convert_to_vaex)
    state.gdx_convert_to_pickle = st.checkbox("12. Convert symbols from GDX to pickle files", state.gdx_convert_to_pickle)
    if any([state.gdx_convert_to_csv, state.gdx_convert_to_pickle , state.gdx_convert_to_vaex]):
        state.gdx_convert_parallel_threads = st.slider("Number of cores for parallel dgx files conversion. To select the maximum number of cores choose zero.", 0, 10, state.gdx_convert_parallel_threads)
    state.report_data = st.checkbox("13. Summarize symbols across scenarios (one table per symbol)", state.report_data)
    if state.report_data:
        if not state.gdx_convert_to_pickle:
            st.error('Error: To summarize symbols across scenarios, pickle files must be active (12.)')
...
def update_proj_var(state):
    ...
    dfn.to_csv(os.path.join(settings.BASE_DIR_ABS,'project_variables.csv'), index=False)
...
def get_results(state):
    ...
    # capacity variables
    symbols['N_TECH'] = Symbol('N_TECH', 'v', 'MW', {'MW': 'Power Capacity [MW]'}, symbol_handler=SH)
    symbols['N_STO_P'] = Symbol('N_STO_P', 'v', 'MW', {'MW': 'Power Capacity [MW]'}, symbol_handler=SH)
    symbols['N_STO_E'] = Symbol('N_STO_E', 'v', 'MWh', {'MWh': 'Energy [MWh]'}, symbol_handler=SH)
    symbols['N_RSVR_P'] = Symbol('N_RSVR_P', 'v', 'MW', {'MW': 'Power Capacity [MW]'}, symbol_handler=SH)
    symbols['N_RSVR_E'] = Symbol('N_RSVR_E', 'v', 'MWh', {'MWh': 'Energy [MWh]'}, symbol_handler=SH)
    symbols['NTC'] = Symbol('NTC', 'v', 'MW', {'MW': 'Installed Capacity [MW]'}, symbol_handler=SH)
    # Generation variables (h)
    symbols['STO_IN'] = Symbol('STO_IN', 'v', 'MW', {'MW': 'Load [MW]'}, symbol_handler=SH)
    symbols['STO_OUT'] = Symbol('STO_OUT', 'v', 'MW', {'MW': 'Load [MW]'}, symbol_handler=SH)
    symbols['G_L'] = Symbol('G_L', 'v', 'MW', {'MW': 'Load [MW]'}, symbol_handler=SH)
    symbols['G_RES'] = Symbol('G_RES', 'v', 'MW', {'MW': 'Load [MW]'}, symbol_handler=SH)
    symbols['RSVR_OUT'] = Symbol('RSVR_OUT', 'v', 'MW', {'MW': 'Load [MW]'}, symbol_handler=SH)
    # Ancillary variables (h)
    symbols['G_INFES'] = Symbol('G_INFES', 'v', 'MW', {'MW': 'Load [MW]'}, symbol_handler=SH)
    symbols['CU'] = Symbol('CU', 'v', 'MW', {'MW': 'Load [MW]'}, symbol_handler=SH)
    symbols['F'] = Symbol('F', 'v', 'MW', {'MW': 'Load [MW]'}, symbol_handler=SH)
    # Parameters
    symbols['phi_res'] = Symbol('phi_res', 'v', '', {'': 'Capacity Factor []'}, symbol_handler=SH)
    symbols['d'] = Symbol('d', 'v', 'MW', {'MW': 'Load [MW]'}, symbol_handler=SH)
    symbols['inc'] = Symbol('inc', 'v', '', {'': 'Incidence Index []'}, symbol_handler=SH)
    # Shadow prices
    symbols['con1a_bal'] = Symbol('con1a_bal', 'm', '€/MW', {'€/MW': 'Shadow Price [€/MW]'}, symbol_handler=SH)
    # Features variables and parameters
    if 'ev_endogenous' in symbols['features']:
        symbols['EV_CHARGE'] = Symbol('EV_CHARGE', 'v', 'MW', {'MW': 'Load [MW]'}, symbol_handler=SH)
        symbols['EV_DISCHARGE'] = Symbol('EV_DISCHARGE', 'v', 'MW', {'MW': 'Load [MW]'}, symbol_handler=SH)
        symbols['eta_ev_in'] = Symbol('eta_ev_in', 'v', '', {'': 'Efficincy []'}, symbol_handler=SH)
...
def generate_df(state):
    options = ['Z', 'N_TECH', 'N_STO_P', 'N_STO_E', 'N_RSVR_P', 'N_RSVR_E', 'NTC',
                'agg_hEV_CHARGE', 'agg_hEV_DISCHARGE', 'agg_hFn', 'agg_hSTO_IN', 'agg_hSTO_OUT',
                'agg_hG_TECHt', 'share']
    ...
    if st.button("Show Plot", key='b_other'):
        if state.chart_type == 'Line':
            fig = px.line(df, x=state.x_gtech, y="value", color=state.color_gtech, facet_row=state.row_gtech, facet_col=state.col_gtech,
                        width=None, height=None)
            # fig.for_each_annotation(lambda a: a.update(text=a.text.split("=")[1]))
            st.plotly_chart(fig,use_container_width=True)
        elif state.chart_type == 'Bar':
            fig = px.bar(df, x=state.x_gtech, y="value", color=state.color_gtech, barmode=state.barmode_gtech, facet_row=state.row_gtech, facet_col=state.col_gtech,
                        width=None, height=None)
            # fig.for_each_annotation(lambda a: a.update(text=a.text.split("=")[1]))
            st.plotly_chart(fig,use_container_width=True)
...
def generate_rldc(state):
    ...
    state.options_ctr = list(state.rldc_table[state.rldc_table['id'] == state.scen]['n'].unique())
    state.country = st.selectbox("Country code:", state.options_ctr, 0)
    df = state.rldc_table.query(f" id == '{state.scen}' & n == '{state.country}'").round(1)
    raw_tech_opt = [tech for tech in df.loc[:, (df != 0).any(axis=0)].columns.to_list() if tech not in ['id','n','h', 'shadow', 'RLDC', 'hr']]
    state.options_tech = [tech for tech in state.rldc_info if tech in raw_tech_opt]
    max_rng = max(list(df['h'].unique()))
    state.tech_order = st.multiselect("Sort technologies to display", state.options_tech, state.options_tech)
    ...
    # keep running forever
        download_button_str = download_button(ch, 'rldc.csv', 'Download', pickle_it=False)
...
def make_symbol_df(symbols):
    ...
    demand_gen = symbols['total_demand'].df.set_index(['id','n']).rename(columns={'value':'Total Demand'}).drop('symbol',axis=1).join(pt.rename(columns={'Renewable':'Renewable','Non-Renewable':'Non-Renewable'}))
    demand_gen.loc[:,'Total Generation'] = demand_gen[['Renewable','Non-Renewable']].sum(axis=1)
    demand_gen.loc[:,'Demand'] = symbols['agg_hD'].df.set_index(['id','n'])['value']
    demand_gen.loc[:,'Storage Losses'] = symbols['storage_losses'].df.set_index(['id','n'])['value']
    demand_gen.loc[:,'Net Import-Export'] = symbols['Fn'].dimreduc('h').dimreduc('l').df.set_index(['id','n'])['value']
    demand_gen.loc[:,'Infeasibility'] = symbols['G_INFES'].dimreduc('h').df.set_index(['id','n'])['value']

    try:
        demand_gen.loc[:,'EV Demand and Losses'] = symbols['ev_demand_losses'].df.set_index(['id','n'])['value']
    except:
        pass
    dem_gen_cols = []
    for header in ['Demand', 'EV Demand and Losses', 'Storage Losses', \
                    'Total Demand', 'Non-Renewable', \
                     'Renewable','Total Generation','Net Import-Export','Infeasibility']:
    ...
    summary.loc[:,'Demand - Gen'] = summary['Total Demand'] - summary['Total Generation']
...
def plotly_table(df):
    ...
    col_elemlen = [max(df[col].apply(lambda x: len(x)).values) if isinstance(df[col].unique()[0], str) else max(df[col].apply(lambda x: len(str(x))).values) for col in df.columns]
    col_ziplen = [max(tup) + 2 if i in [0, 1] else max(tup) for i, tup in enumerate(zip(col_strlen,col_elemlen))]
